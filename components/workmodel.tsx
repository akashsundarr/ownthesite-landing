"use client";

import { FadeUp } from "./animations";

const CheckIcon = ({ className }) => (
  <svg
    className={`w-5 h-5 flex-shrink-0 ${className}`}
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
    strokeWidth={2.5}
  >
    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
  </svg>
);

export function WorkModel() {
  return (
    <section id="pricing" className="py-28 px-6 bg-white">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="text-center mb-16">
          <FadeUp>
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-black mb-4">
              Simple process. Clear costs.
            </h2>
          </FadeUp>

          <FadeUp delay={0.1}>
            <p className="text-base text-gray-500 max-w-md mx-auto">
              One-time website development with optional hosting and maintenance support.
            </p>
          </FadeUp>
        </div>

        {/* Cards */}
        <div className="grid md:grid-cols-2 gap-8 items-stretch">
          {/* Option 1: Website Development */}{" "}
          <FadeUp>
            <div className="p-8 md:p-10 rounded-3xl border border-gray-200 h-full flex flex-col hover:border-gray-300 transition-colors duration-300">
              <div className="mb-8">
                <h3 className="text-2xl font-bold text-black tracking-tight">
                  Website Development
                </h3>
                <p className="text-sm text-gray-500 mt-2 font-medium">
                  A custom website built for your business.
                </p>

                <div className="mt-6 pt-6 border-t border-gray-100">
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-extrabold text-black">
                      30%
                    </span>
                    <span className="text-gray-500 font-medium">
                      upfront, 70% on delivery
                    </span>
                  </div>
                </div>
              </div>
              <ul className="space-y-4 text-gray-600 flex-1">
                <li className="flex items-start gap-3">
                  <CheckIcon className="text-black mt-0.5" />
                  <span>Custom design tailored to your business</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-black mt-0.5" />
                  <span>Mobile-friendly and responsive on all devices</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-black mt-0.5" />
                  <span>WhatsApp integration and basic SEO setup</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-black mt-0.5" />
                  <span>Domain connection and website launch support</span>
                </li>
              </ul>
              <button className="mt-10 w-full py-3.5 rounded-xl border border-gray-200 text-black font-semibold hover:bg-gray-50 hover:border-gray-300 transition-all duration-200">
                Get Started
              </button>
            </div>
          </FadeUp>
          {/* Option 2: Hosting & Maintenance */}{" "}
          <FadeUp delay={0.1}>
            <div className="relative p-8 md:p-10 rounded-3xl bg-black text-white h-full flex flex-col shadow-2xl shadow-gray-900/20">
              <div className="absolute -top-3.5 left-8">
                <span className="bg-white text-neutral-700 text-xs px-4 py-1.5 rounded-full font-bold tracking-wide uppercase">
                  Optional
                </span>
              </div>
              <div className="mb-8">
                <h3 className="text-2xl font-bold text-white tracking-tight">
                  Hosting & Maintenance
                </h3>
                <p className="text-sm text-gray-400 mt-2 font-medium">
                  Ongoing support to keep your website running smoothly.
                </p>

                <div className="mt-6 pt-6 border-t border-gray-800">
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-extrabold text-white">
                      Optional
                    </span>
                    <span className="text-gray-400 font-medium">
                      yearly support
                    </span>
                  </div>
                </div>
              </div>
              <ul className="space-y-4 text-gray-300 flex-1">
                <li className="flex items-start gap-3">
                  <CheckIcon className="text-white mt-0.5" />
                  <span>Hosting and deployment management</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-white mt-0.5" />
                  <span>Security updates and website backups</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-white mt-0.5" />
                  <span>Content edits and small website changes</span>
                </li>

                <li className="flex items-start gap-3">
                  <CheckIcon className="text-white mt-0.5" />
                  <span>Priority support whenever needed</span>
                </li>
              </ul>
              <button className="mt-10 w-full py-3.5 rounded-xl bg-white text-black font-bold hover:bg-gray-100 hover:scale-[1.02] transition-all duration-200">
                Learn More
              </button>
            </div>
          </FadeUp>
        </div>
      </div>
    </section>
  );
}
